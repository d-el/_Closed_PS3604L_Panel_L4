...
void baseTSK(void *pPrm){
    ...
    portTickType    		IdleTime = xTaskGetTickCount();
    ...
    grf_line(0, 107, 159, 107, half_light_gray);
...
void printStatusBar(void){
    ...
    if((fp.tf.state.bit.errorLinearRegTemperSens != 0)||
       (fp.tf.state.bit.ovfLinearRegTemper != 0) ||
	   (fp.tf.state.bit.reverseVoltage != 0)||
