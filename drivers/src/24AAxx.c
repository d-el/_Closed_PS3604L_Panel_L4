...
* @param    len - number bytes of read
* @retval   none
*/
eepStatus_type eep_write(uint16_t dst, void *src, uint16_t len){
    ...
    while(len > 0){
        ...
        if(len <= canWrite){
            ...
            i2c_write(usei2c, eep_tempBff, len + 1, eepAdr.all, i2cNeedStop);  //Пишем len байтов
...
* @param    src - linear address (0 - 1024 for 24AA08)
* @param    len - number bytes of read
* @retval   none
*/
eepStatus_type eep_read(void *dst, uint16_t src, uint16_t len){
    ...
    i2c_write(usei2c, &adrInBlock, 1, eepAdr.all, i2cNeedStop);  //Пишем len байтов
    while(usei2c->state == i2cTxRun);
    if(usei2c->state != i2cTxSuccess){
        return eepI2cError;
    }
