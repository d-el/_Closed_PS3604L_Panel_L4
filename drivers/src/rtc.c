...
rtcStatus_type rtc_init(void){
    ...
    if((RCC->BDCR & RCC_BDCR_RTCEN) == 0){
        ...
        while(((RCC->BDCR & RCC_BDCR_LSEON) == 0)&&
              (timeout != 0));                          //Wait for enabled LSE
        if(timeout == 0){
            return rtc_error;
        }
        ...
        while((RTC->ISR & RTC_ISR_INITF) == 0);         //Wait for calendar registers update is allowed
...
void rtc_getTime(struct tm 	*time){
    ...
    time->tm_sec =	((rtctr & RTC_TR_SU) >> RTC_TR_SU_Pos) +
                	((rtctr & RTC_TR_ST) >> RTC_TR_ST_Pos) * 10;

    time->tm_min =	((rtctr & RTC_TR_MNU) >> RTC_TR_MNU_Pos) +
                	((rtctr & RTC_TR_MNT) >> RTC_TR_MNT_Pos) * 10;

    time->tm_hour =	((rtctr & RTC_TR_HU) >> RTC_TR_HU_Pos) +
                	((rtctr & RTC_TR_HT) >> RTC_TR_HT_Pos) * 10;

    time->tm_mday =	((rtcdr & RTC_DR_DU) >> RTC_DR_DU_Pos) +
                	((rtcdr & RTC_DR_DT) >> RTC_DR_DT_Pos) * 10;

    time->tm_mon =	((rtcdr & RTC_DR_MU) >> RTC_DR_MU_Pos) +
                	((rtcdr & RTC_DR_MT) >> RTC_DR_MT_Pos) * 10;

    time->tm_wday = ((rtcdr & RTC_DR_WDU) >> RTC_DR_WDU_Pos);

    time->tm_year =	((rtcdr & RTC_DR_YU) >> RTC_DR_YU_Pos) +
                	((rtcdr & RTC_DR_YT) >> RTC_DR_YT_Pos) * 10
					+ 100;
